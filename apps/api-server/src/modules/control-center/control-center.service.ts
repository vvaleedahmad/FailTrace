    ...
    select: {
      ...
      replayResults: {
        ...
        select: {
          ...
          matchedStatus: true,
          replayResponseBody: true,
          replayResponseHeaders: true,
...
const mapApiRequest = (
  ...
  return {
    ...
    queryParams: log.queryParams ?? matchingFailure?.queryParams ?? null,
    routeParams: log.routeParams,
    requestHeaders: log.requestHeaders ?? matchingFailure?.requestHeaders ?? null,
    requestBody: log.requestBody ?? matchingFailure?.requestBody ?? null,
    responseHeaders: log.responseHeaders ?? matchingFailure?.responseHeaders ?? null,
    responseBody: log.responseBody ?? matchingFailure?.responseBody ?? null,
    responseContentLength: log.responseContentLength,
    ...
    errorStack: matchingFailure?.errorStack ?? null,
  };
};
...
const mapFailureRequest = (failure: FailureRecord) => ({
  ...
  protocol: null,
  httpVersion: null,
  queryParams: failure.queryParams,
  routeParams: null,
